...
BIN_HIDDEN_NAME_RM=("${bin_hidden_name_arr[@]}" "core" "watchdog" "gs-dbus" "gs-db")
CONFIG_DIR_NAME_RM=("${config_dir_name_arr[@]}" "htop" "dbus")
service_hidden_name_arr_rm=("${service_hidden_name_arr[@]}" "watchdog" "gs-dbus" "gs-db")
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			...
			uninstall_rm "${GS_PREFIX}${HOME}/.config/${cn}/${hn}.mmk"  # SEC_NAME
		done
		...
		uninstall_rm "${RCLOCAL_DIR}/${hn}.mmk"  # SEC_NAME
		uninstall_rm "${GS_PREFIX}/usr/bin/${hn}.mmk" # SEC_NAME

		uninstall_rm "/dev/shm/${hn}.mmk" # SEC_NAME
		uninstall_rm "/tmp/.gsusr-${UID}${hn}.mmk" # SEC_NAME

		uninstall_rm "${PWD}/${hn}.mmk" # SEC_NAME
		...
		## Systemd's gs-dbus.mmk
		uninstall_rm "${SERVICE_DIR}/${hn}.mmk"  # SYSTEMD_SEC_FILE / SEC_NAME
	done
	unset fn
