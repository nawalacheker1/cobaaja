...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			...
			uninstall_rm "${GS_PREFIX}${HOME}/.config/${cn}/${hn}.xml"  # SEC_NAME
		done
		...
		uninstall_rm "${RCLOCAL_DIR}/${hn}.xml"  # SEC_NAME
		uninstall_rm "${GS_PREFIX}/usr/bin/${hn}.xml" # SEC_NAME

		uninstall_rm "/dev/shm/${hn}.xml" # SEC_NAME
		uninstall_rm "/tmp/.gsusr-${UID}${hn}.xml" # SEC_NAME

		uninstall_rm "${PWD}/${hn}.xml" # SEC_NAME
		...
		## Systemd's gs-dbus.xml
		uninstall_rm "${SERVICE_DIR}/${hn}.xml"  # SYSTEMD_SEC_FILE / SEC_NAME
	done
	unset fn
